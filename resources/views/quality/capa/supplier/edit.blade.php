        ...
        <tbody>
            @forelse($capas ?? [] as $index => $data)
                <tr>
                    ...
                    {{-- PERUBAHAN DI SINI: Logika tombol untuk melihat data isian --}}
                    <td>
                        @if($data->status == 'Review' || $data->status == 'Closed')
                            <a href="{{ url('/capa-8d/supplier/detail/' . $data->id) }}" target="_blank" class="btn-aksi" style="background-color: #0284c7; color: white;">
